...
describe("Phase 5 fifth-review regressions", () => {
  ...
  test("two rapid direct Create calls enqueue only one transition and disable actual New/select controls", async () => {
    ...
    expect(screen.getByRole("button", { name: "Renombrar conversación" })).toBeDisabled();
    expect(screen.getByRole("button", { name: "Archivar conversación" })).toBeDisabled();
    expect(screen.getByRole("button", { name: "Eliminar conversación" })).toBeDisabled();
    releaseCreate();
