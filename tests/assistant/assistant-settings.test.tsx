...
describe("Assistant provider settings", () => {
  ...
  test("refreshes a complete catalog without compatibility probes", async () => {
    const factory = new IDBFactory(); vi.stubGlobal("IDBKeyRange", IDBKeyRange);
    const calls: unknown[] = [];
    vi.stubGlobal("fetch", vi.fn(async (_url, init) => { const body = JSON.parse(String(init?.body)); calls.push(body); if (body.operation === "catalog") return Response.json({ completion: "complete", models: [{ id: `${body.providerId}:model`, providerId: body.providerId, canonicalModelId: "model", apiModelId: "model", generationModelId: "model", displayName: "Model", capabilities: { chat: true, tools: "unknown", streaming: true, vision: false, documents: false }, availability: "available", metadataSource: "official", detectedAt: new Date().toISOString() }] }); return Response.json({ providerId: body.config?.id ?? body.providerId, keyStatus: "configured" }); }));
