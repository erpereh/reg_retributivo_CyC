...
import { buildPersonAnalysisPresentation, personAnalysisEvidenceSchema, personAnalysisExcerpt, type PersonAnalysisEvidence } from "@/lib/assistant/tools/personEvidence";
...
const legacyProfile = z.object({ personId, workplace: z.string().optional(), position: z.string().optional(), category: z.string().optional(), totals: moneyTriple, blocks: z.object({ salary: moneyTriple, salaryComplement: moneyTriple, extraSalary: moneyTriple }).strict(), status, periods: z.array(z.string()) }).strict();
...
const outputSchemas: Record<AnalysisToolName, z.ZodTypeAny> = {
  getAnalysisSummary: z.object({ summary }).strict(), findPersonByEmployeeId: z.object({ person: safePerson.optional() }).strict(), searchPeople: z.object({ people: z.array(safePerson) }).strict(), getPersonProfile: z.union([personAnalysisEvidenceSchema, legacyProfile]),
...
export function createAnalysisToolRegistry(context: AnalysisToolRegistryContext): AnalysisToolRegistry {
  ...
  return { names: ANALYSIS_TOOL_NAMES, privacyBlockedTerms, execute, assertSafeOutput(value) { assertNoKnownNames(value, allCanonicalNames); assertSafeForProvider(value); }, async executeEnvelope(name, args, requestId = "local") {
    ...
    if (!sources.length && name === "getPersonProfile") {
      const evidence = data as PersonAnalysisEvidence;
      const presentation = buildPersonAnalysisPresentation(evidence);
      const excerpt = personAnalysisExcerpt(evidence);
      const factKey = `${name}:${requestId}`;
      const sanitizedHash = await sha256(canonicalJson({ tool: name, requestId, data, presentation, excerpt, factKey }));
      sources.push({ id: `tool-source-${sanitizedHash}`, conversationId: context.conversation.id, analysisId: parsed.analysisId, personId: evidence.personId, sourceType: "person_analysis", sanitizedSourceLabel: `Evidencia retributiva · matrícula ${evidence.personId}`, availability: "available", conceptIds: evidence.comparisons.map((item) => item.registroCode), excerpt, sanitizedHash, presentation });
    }
    if (!sources.length) {
