...
export interface RequestedLocalTool { readonly requestId: string; readonly tool: AnalysisToolName; readonly args: unknown }
...
export async function executeAtomicToolRound(registry: AnalysisToolRegistry, requests: readonly RequestedLocalTool[], options: Readonly<{ signal: AbortSignal; timeoutMs?: number }>): Promise<readonly SettledLocalTool[]> {
  ...
  return Promise.all(requests.map(async (request): Promise<SettledLocalTool> => {
    ...
    try {
      ...
      return { requestId: request.requestId, tool: request.tool, args: request.args, status: empty ? "empty" : "success", data: envelope.data, sources: envelope.sources };
    } catch {
      const cancelled = controller.signal.aborted;
      return { requestId: request.requestId, tool: request.tool, args: request.args, status: cancelled ? "cancelled" : "failed", error: { code: cancelled ? "cancelled" : "tool_failed", message: "No se pudo completar la consulta local." }, sources: [] };
