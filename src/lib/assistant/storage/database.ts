...
export function migrateAssistantDatabase(db: IDBDatabase, transaction: IDBTransaction): void {
  ...
  const profiles = transaction.objectStore("modelProfiles");
  const cursor = profiles.openCursor();
  cursor.onsuccess = () => {
    const current = cursor.result;
    if (!current) return;
    const value = current.value as Record<string, unknown>;
    const migrated: Record<string, unknown> = { ...value, detectedModels: Array.isArray(value.detectedModels) ? value.detectedModels : [] };
    delete migrated.selectedModelCatalogId;
