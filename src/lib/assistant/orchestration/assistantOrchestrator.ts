...
export class AssistantOrchestrator {
  ...
  private async execute(input: SendAssistantInput, beforeRun?: (context: RunWriteContext) => Promise<void> | undefined): Promise<AssistantRunResult> {
    ...
    try {
      ...
      let continuation: { messageId: string; context: string } | undefined = input.analysisId ? input.resumeFrom : undefined; let lastError: unknown; let attempt = 0; let posts = 0; let accumulatedText = ""; let phase: "general" | "plan" | "respond" | "continue" = input.analysisId ? (continuation ? "continue" : "plan") : "general"; let toolResults: unknown[] = [];
      while (attempt < profiles.length && posts < 3) {
        ...
        try {
          ...
          if (classification !== "transient") throw error; attempt += 1; phase = input.analysisId ? (continuation ? "continue" : "plan") : "general"; toolResults = [];
        }
      }
