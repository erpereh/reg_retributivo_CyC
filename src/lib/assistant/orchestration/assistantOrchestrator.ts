...
export class AssistantOrchestrator {
  ...
  retry() { if (!this.lastInput) return Promise.reject(new Error("No hay una solicitud para reintentar.")); return this.send({ ...this.lastInput, resumeFrom: this.lastInterrupted }); }
  async regenerate() { if (!this.lastInput) throw new Error("No hay una respuesta para regenerar."); const input = { ...this.lastInput, resumeFrom: undefined }; const replacedId = this.lastProducedMessageId; return this.execute(input, replacedId ? (context) => this.dependencies.markMessageReplaced?.(replacedId, context) : undefined); }
  ...
  private async execute(input: SendAssistantInput, beforeRun?: (context: RunWriteContext) => Promise<void> | undefined): Promise<{ readonly text: string; readonly events: readonly AssistantStreamEvent[]; readonly rounds: number }> {
    ...
    try {
      ...
      if (controller.signal.aborted) throw new AssistantRunStoppedError(allEvents.filter((event): event is Extract<AssistantStreamEvent, { type: "text_delta" }> => event.type === "text_delta").map((event) => event.delta).join(""), allEvents);
      throw error;
