          ...
          <>
            <section className="space-y-3" aria-labelledby="assistant-preferences">
              ...
              <label className="block text-xs font-semibold text-muted">Modelo
                <select className="filter-control mt-1" aria-label="Modelo de conversación" value={conversation.modelProfileId} onChange={(event) => void assistant.updateConversationPreferences({ modelProfileId: event.target.value })}>
              ...
              <label className="block text-xs font-semibold text-muted">Modo de respuesta
                <select className="filter-control mt-1" aria-label="Modo de respuesta" value={conversation.responseMode} onChange={(event) => void assistant.updateConversationPreferences({ responseMode: event.target.value as "strict" | "flexible" })}>
              ...
              <label className="block text-xs font-semibold text-muted">Estrategia de contexto
                <select className="filter-control mt-1" aria-label="Estrategia de contexto" value={conversation.contextStrategy} onChange={(event) => void assistant.updateConversationPreferences({ contextStrategy: event.target.value as "automatic" | "full" | "optimized" })}>
            ...
            {conversation.type === "analysis" ? (
              <section className="border-t border-line pt-4" aria-labelledby="people-context-title">
                ...
                />
                {conversation.primaryPersonId ? <button type="button" className="btn-secondary mt-3 w-full" disabled={assistant.streaming} onClick={() => void assistant.requestPersonProfile()}>Consultar perfil</button> : null}
              </section>
            ) : null}
