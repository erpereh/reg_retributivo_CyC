...
export function AssistantShell({ assistant }: Readonly<{ assistant: AssistantContextValue }>) {
  ...
  const conversationPanel = <ConversationSidebar
    ...
    onArchive={() => void assistant.archiveConversation()}
    onDelete={() => void assistant.deleteConversation()}
  />;
  const contextPanel = <ContextSidebar assistant={assistant} />;

  return (
    <section ref={chatFallback} tabIndex={-1} aria-label="Área de conversación" data-testid="assistant-shell" className="min-h-[calc(100dvh-7rem)] overflow-hidden rounded-2xl bg-white shadow-soft ring-1 ring-line/80">
      <div className="flex min-h-14 items-center gap-2 border-b border-line bg-white px-3 xl:hidden">
        <button ref={conversationsTrigger} type="button" className="inline-flex min-h-11 min-w-11 items-center gap-2 rounded-xl px-3 text-sm font-bold text-ink hover:bg-slate-100" aria-label="Abrir conversaciones" aria-expanded={drawer === "conversations"} onClick={() => setDrawer("conversations")}><MessageSquareText aria-hidden="true" className="size-4" /><span className="hidden sm:inline">Conversaciones</span></button>
      ...
      <div className="grid h-[calc(100dvh-10.5rem)] min-h-[34rem] min-w-0 grid-cols-1 lg:grid-cols-[minmax(0,1fr)_19rem] xl:grid-cols-[17rem_minmax(0,1fr)_19rem]">
        <div className="hidden min-h-0 overflow-hidden border-r border-line xl:block">{conversationPanel}</div>
        <ConversationTimeline assistant={assistant} />
        <div className="hidden min-h-0 overflow-hidden border-l border-line lg:block">{contextPanel}</div>
      </div>
      ...
      <AssistantDrawer open={drawer === "context"} title="Contexto" side="right" onClose={closeDrawer}>{contextPanel}</AssistantDrawer>
    </section>
  );
}
