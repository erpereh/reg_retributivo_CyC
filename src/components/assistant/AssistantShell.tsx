...
export function AssistantShell({ assistant }: Readonly<{ assistant: AssistantContextValue }>) {
  ...
  return <section ref={chatFallback} tabIndex={-1} aria-label="Area de conversacion" data-testid="assistant-shell" className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-2xl bg-white shadow-soft ring-1 ring-line/80">
    ...
    <div className="grid min-h-0 min-w-0 flex-1 grid-cols-1 lg:grid-cols-[minmax(0,1fr)_19rem] xl:grid-cols-[17rem_minmax(0,1fr)_19rem]"><div className="hidden min-h-0 overflow-hidden border-r border-line xl:block">{conversationPanel}</div><ConversationTimeline assistant={assistant} onShowContextUsage={showContextUsage} /><div className="hidden min-h-0 overflow-hidden border-l border-line lg:block">{contextPanel(drawer !== "context", drawer !== "context")}</div></div>
    <AssistantDrawer open={drawer === "conversations"} title="Conversaciones" side="left" onClose={closeDrawer}>{conversationPanel}</AssistantDrawer><AssistantDrawer open={drawer === "context"} title="Contexto" side="right" onClose={closeDrawer}>{contextPanel(drawer === "context", drawer === "context")}</AssistantDrawer>
  </section>;
}
