...
export function AssistantProvider({ children, activeAnalysis, factory, dbName, adapter, repositoriesFactory, onNavigate }: Readonly<{
  ...
  const conversationUsesActiveAnalysis = Boolean(conversation?.type === "analysis" && activeAnalysis && activeAnalysis.id === conversation.analysisId);
  const activeAnalysisPeople = conversationUsesActiveAnalysis ? activeAnalysisResult?.people : undefined;
  const availablePersonIds = useMemo(() => conversationUsesActiveAnalysis
    ? (activeAnalysisPeople ?? []).map((person) => person.employeeNumber) : [], [activeAnalysisPeople, conversationUsesActiveAnalysis]);
